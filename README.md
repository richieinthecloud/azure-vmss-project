...
- An internal load balancer handles traffic from our Web tier VMSS to our App tier VMSS. NSGs at this tier only allow traffic from our App tier to the SQL database instance. The SQL database in this case is connecting via a Private Endpoint subnet. 
...
- Our VMSS always starts with 1 default instance. Dependent on the environment (Prod, Dev or DR) it can autoscale to 3 or 4 instances. Crossing 70% CPU for 5 minutes will trigger a scale out. A CPU threshold of 40% or less will make our instances scale back down. Minimum instance being 1, no matter the environment
